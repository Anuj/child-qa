    ...
    if (size == capacity) {
      ...
      grow(2 * capacity + 1);
    		...
    		for (int i = 0; i < size; ++i) {
    			E currKey = (E)elements[i];
    			for (int j = 0; j < size; ++j) {
    				if ((currKey.equals((E)elements[j])) && i != j) {
    					System.out.println("NOOOO " + currKey.toString() + " " + ((E)elements[j]).toString());
    					currKey.equals((E)elements[j]);
    				}
    				assert(currKey.equals((E)elements[j]));
    				//if (currKey.equals((E)elements[j]_))
...
//        sb.append("\n");
  ...
  public Counter asCounter(int nElements) {
	    PriorityQueue<E> pq = deepCopy();
	    Counter<E> counter = new Counter<E>();
	    int iElement = 0;
	    while (! pq.isEmpty() && iElement < nElements) {
	      double priority = pq.getPriority();
	      E element = pq.removeFirst();
	      counter.incrementCount(element, priority);
	      ++iElement;
  ...
  public void setPriorities(Counter<E> c) {
	  for (Entry<E,Double> e : c.getEntrySet())
		  setPriority(e.getKey(), e.getValue());
  }
  ...
  public FastPriorityQueue(Counter<E> c) {
	  for (Entry<E,Double> e : c.getEntrySet())
		  setPriority(e.getKey(), e.getValue());
  }
